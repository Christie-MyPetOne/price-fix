...
import CalculadoraMargem from "../Vendas/Calculadora"; // ✅ importa a calculadora
...
export function VendasTable() {
  ...
  const [vendaSelecionada, setVendaSelecionada] = useState<(typeof sales)[0] | null>(null);
  ...
  const headers: { key: keyof (typeof sales)[0]; label: string }[] = [
    { key: "orderDate", label: "Pedido" },
    { key: "customer", label: "Cliente" },
    { key: "totalAmount", label: "Valor da venda" },
  ];
            ...
            {displayedSales.map((sale) => (
              <tr key={sale.id} className="hover:bg-background transition-colors">
                ...
                <td className="px-4 py-3 text-sm font-medium text-text flex flex-col gap-1">
                  ...
                  <span><strong>Data:</strong> {new Date(sale.orderDate).toLocaleDateString("pt-BR")}</span>
                  <span><strong>Produto:</strong> {sale.items.map((item) => item.productName).join(", ")}</span>
                </td>
