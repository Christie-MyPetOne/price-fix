...
function calcPedido(d: VendasInfoData) {
  const receita =
    safe(d.receitaProdutos) + safe(d.receitaFrete) - safe(d.descontos);

  const variaveisComponentes =
    safe(d.custoProdutos) +
    safe(d.comissaoCanal) +
    safe(d.taxaFixaMktplace) +
    safe(d.custoEnvio) +
    safe(d.taxaFrete);
  const variaveisCampo = safe(d.custosVariaveis);
  const variaveisUsado =
    variaveisCampo > 0 ? variaveisCampo : variaveisComponentes;

  const adicionais =
    safe(d.gastosFixosPedido) +
    safe(d.gastosFreteAdicionais) +
    safe(d.descontosAdicionais) +
    safe(d.impostosAdicionais);
...
function calcItem(p: ProdutoDaVenda) {
  ...
  const variaveisComponentes =
    safe(p.custoProdutos) +
    safe(p.comissaoCanal) +
    safe(p.taxaFixaMktplace) +
    safe(p.custoEnvio) +
    safe(p.taxaFrete);
  ...
  const adicionais =
    safe(p.gastosFixosPedido) +
    safe(p.gastosFreteAdicionais) +
    safe(p.descontosAdicionais) +
    safe(p.impostosAdicionais);
...
const Collapsible: React.FC<{
  ...
  useEffect(() => {
    if (openAll !== null) setOpen(openAll);
  }, [openAll]);
  return (
    <div style={{ ...card, padding: 0 }}>
      ...
      {open && children && (
        <div style={{ padding: 16, paddingTop: 0 }}>{children}</div>
      )}
    </div>
  );
};
      ...
      >
        ...
        <div style={{ padding: 16 }}>
          <div style={{ ...card }}>
            {/* KPIs principais */}
            <div
              style={{ padding: 16, ...gridCols(4), gap: 12, marginBottom: 12 }}
            >
              <div style={chip}>
            ...
            <div style={{ padding: 16 }}>
              <Collapsible
                title="Receita, custos e impostos"
                openAll={openAll}
                defaultOpen
              >
                <div
                ...
                >
                  ...
                  <div style={card}>
                    ...
                    >
                      receita_total = {fmtBRL(data.receitaProdutos)} +{" "}
                      {fmtBRL(data.receitaFrete)} − {fmtBRL(data.descontos)} ={" "}
                      <b>{fmtBRL(K.receita)}</b>
                    </div>
                  </div>
                  ...
                  <div style={card}>
                    ...
                    >
                      subtotal_componentes ={" "}
                      <b>{fmtBRL(K.variaveisComponentes)}</b> | campo agregado ={" "}
                      <b>{fmtBRL(data.custosVariaveis)}</b>
                      <br />
                      usado no cálculo = <b>{fmtBRL(K.variaveisUsado)}</b>{" "}
                      {K.variaveisCampo > 0
                        ? "(campo agregado)"
                  ...
                  <div style={card}>
                    ...
                    <div style={row}>
                      <span style={{ color: "var(--color-text-secondary)" }}>
                        Gastos fixos do pedido
                      </span>
                      <strong>{fmtBRL(data.gastosFixosPedido)}</strong>
                    </div>
                    <div style={row}>
                      <span style={{ color: "var(--color-text-secondary)" }}>
                        Gastos de frete adicionais
                      </span>
                ...
                <div style={{ ...card, marginTop: 12 }}>
                  ...
                  <div style={{ fontSize: 14 }}>
                    ...
                    <div style={{ marginBottom: 6 }}>
                      Custos totais:{" "}
                      <b>
                        {fmtBRL(K.variaveisUsado + K.impostos + K.adicionais)}
                      </b>{" "}
                      <span style={{ color: "var(--color-text-secondary)" }}>
                        (variáveis + impostos + adicionais)
                      </span>
                    </div>
                    <div>
                      Lucro: <b>{fmtBRL(K.lucro)}</b> &nbsp;|&nbsp; Margem:{" "}
                      <b>{fmtPct(K.margem)}</b>
                    </div>
                  </div>
                </div>
                    ...
                    >
                      ...
                      <div style={card}>
                        ...
                        <div style={row}>
                          ...
                          <strong>{fmtBRL(p.precoVenda)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.receitaFrete)}</strong>
                        </div>
                        <div style={{ ...row, borderBottom: "none" }}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                        ...
                        >
                          receita_item = {fmtBRL(p.precoVenda)} +{" "}
                          {fmtBRL(p.receitaFrete)} − {fmtBRL(p.desconto)} ={" "}
                          <b>{fmtBRL(I.receita)}</b>
                        </div>
                      </div>
                      ...
                      <div style={card}>
                        ...
                        <div style={row}>
                          ...
                          <strong>{fmtBRL(p.custoProdutos)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.comissaoCanal)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.taxaFixaMktplace)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.custoEnvio)}</strong>
                        </div>
                        <div style={{ ...row, borderBottom: "none" }}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.taxaFrete)}</strong>
                        </div>
                        <div
                          style={{
                            marginTop: 10,
                        ...
                        >
                          subtotal_componentes ={" "}
                          <b>{fmtBRL(I.variaveisComponentes)}</b> | campo
                          agregado = <b>{fmtBRL(p.custosVariaveis)}</b>
                          <br />
                          usado = <b>{fmtBRL(I.variaveisUsado)}</b>{" "}
                          {I.variaveisCampo > 0
                            ? "(campo agregado)"
                            : "(soma dos componentes)"}
                        </div>
                      ...
                      <div style={card}>
                        <div style={mutedTitle}>
                          Impostos e adicionais (item)
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.impostos)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.gastosFixosPedido)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.gastosFreteAdicionais)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.descontosAdicionais)}</strong>
                        </div>
                        <div style={{ ...row, borderBottom: "none" }}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.impostosAdicionais)}</strong>
                        </div>
                        <div
                          style={{
                            marginTop: 10,
                        ...
                        >
                          custos_totais_item = {fmtBRL(I.variaveisUsado)} +{" "}
                          {fmtBRL(I.impostos)} + {fmtBRL(I.adicionais)} ={" "}
                          <b>{fmtBRL(I.custosTotais)}</b>
                          <br />
                          lucro_item = {fmtBRL(I.receita)} −{" "}
                          {fmtBRL(I.custosTotais)} = <b>{fmtBRL(I.lucro)}</b>
                        </div>
                      </div>
                    </div>
                    ...
                    {p.impostosDetalhe && (
                      <div style={{ ...card, marginTop: 12 }}>
                        ...
                        <div style={row}>
                          ...
                          <strong>{fmtBRL(p.impostosDetalhe.ipiVenda)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.impostosDetalhe.ipi)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>
                            {fmtBRL(p.impostosDetalhe.creditoIPI)}
                          </strong>
                        </div>
                        <div style={row}>
                          <span
                          ...
                          <strong>
                            {fmtBRL(p.impostosDetalhe.creditoICMS)}
                          </strong>
                        </div>
                        <div style={row}>
                          <span
                          ...
                          <strong>{fmtBRL(p.impostosDetalhe.icms)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>{fmtBRL(p.impostosDetalhe.icmsFCP)}</strong>
                        </div>
                        <div style={row}>
                          <span
                            style={{ color: "var(--color-text-secondary)" }}
                          ...
                          <strong>
                            {fmtBRL(p.impostosDetalhe.creditoPIS)}
                          </strong>
                        </div>
                        <div style={{ ...row, borderBottom: "none" }}>
                          <span
                          ...
                          <strong>{fmtBRL(p.impostosDetalhe.cofins)}</strong>
                        </div>
                        <div
                          style={{
                            marginTop: 8,
                        ...
                        >
                          ST:{" "}
                          <b>{p.impostosDetalhe.possuiST ? "Sim" : "Não"}</b> ·
                          Outros créditos:{" "}
                          <b>{fmtBRL(p.impostosDetalhe.outrosCreditos)}</b>
                        </div>
                      </div>
                    )}
                  </Collapsible>
          ...
          <Collapsible title="Outras informações" openAll={openAll} defaultOpen>
            <div style={{ ...card }}>
              <div style={row}>
                ...
                <strong>{data.nfeEmitida ? "Sim" : "Não"}</strong>
              </div>
              <div style={{ ...row, borderBottom: "none" }}>
                <span style={{ color: "var(--color-text-secondary)" }}>
                  Estado de destino
                </span>
                <strong>{data.estadoDestino ?? "—"}</strong>
              </div>
            </div>
          </Collapsible>
        </div>
