...
export default function IntegracaoTinyPage() {
  // -------- Form states --------
    ...
    <div className="max-w-7xl -mt-8 mx-auto w-full flex flex-col gap-6 h-full px-3 sm:px-4 md:px-6 pb-10">
      ...
      <div className="rounded-xl bg-gradient-to-r border border-border/60 mt-4 p-4 sm:p-5 flex flex-col md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-bold text-text">Integração Tiny</h1>
          <p className="text-sm text-text-secondary max-w-3xl">
            Conecte sua conta Tiny para importação automática de vendas, indicadores em tempo real e atualização de preços.
          </p>
        </div>
...
const Toggle = ({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) => (
  ...
  >
    ...
    />
    <span className={classNames("absolute text-center", value ? "left-2" : "right-2")}>
      {value ? "SIM" : "NÃO"}
    </span>
  </button>
);
...
function MultiSelect({
  ...
  const toggle = (opt: Option) => {
    const exists = value.some((o: Option) => o.value === opt.value);
    onChange(exists ? value.filter((o: Option) => o.value !== opt.value) : [...value, opt]);
  };

  const remove = (optValue: string) => onChange(value.filter((o: Option) => o.value !== optValue));
        ...
        {open && (
          <div className="absolute z-50 mt-2 w-full rounded-lg border border-border-dark bg-card shadow-lg max-h-56 overflow-auto p-1.5">
            {options.map((opt: Option) => {
              const active = value.some((v: Option) => v.value === opt.value);
              return (
                <button
                  key={opt.value}
                  onClick={() => toggle(opt)}
