...
export const useSalesStore = create<SalesState>((set, get) => ({
  ...
  filterSales: (filters) => {
    ...
    const filtered = base.filter((s) => {
      ...
      const haystack = `${s.id} ${s.status} ${s.customer?.firstName ?? ""} ${
        s.customer?.lastName ?? ""
      } ${(s.items ?? []).map((i) => i.productName).join(" ")}`.toLowerCase();

      const okQ = filters.q
        ? haystack.includes(String(filters.q).toLowerCase())
        : true;
      const okDate = filters.date
        ? ymdLocal(s.orderDate) === filters.date
        : true;
      const okEmpresa = filters.empresa
        ? (s as any).empresa === filters.empresa
        : true;
      const okCanal = filters.canal ? (s as any).canal === filters.canal : true;
      const okProduto = filters.produto
        ? (s.items ?? []).some((i) => i.productName === filters.produto)
        : true;
